...
foreach (['paths.php','.htaccess'] as $file) {
    $content = file_get_contents("$base_dir/file_storage/$file");
    $content = preg_replace('/links-[0-9a-f]+/',"$links_path",$content);
    file_put_contents("$base_dir/file_storage/$file",$content);
}
print("Links URL is now $base_url/file_storage/$links_path$eol");
