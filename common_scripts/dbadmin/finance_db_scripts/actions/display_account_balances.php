...
$balances = array();
$query_result = mysqli_query($db,"SELECT * FROM accounts WHERE label IS NOT NULL $account_exclusions");
while ($row = mysqli_fetch_assoc($query_result))
{
	$account = $row['label'];
	$query_result2 = mysqli_query($db,"SELECT * FROM funds WHERE type='localised'");
...
$query_result = mysqli_query($db,"SELECT * FROM accounts WHERE label IS NOT NULL $account_exclusions ORDER BY sort_order ASC");
while ($row = mysqli_fetch_assoc($query_result))
{
	$account = $row['label'];
	...
	$query_result2 = mysqli_query($db,$query);
	while ($row2 = mysqli_fetch_assoc($query_result2))
	{
		if ($row2['fund'] != '-split-')
		{
	...
	foreach($balances as $key => $value)
	{
		if (strtok($key,'#') == $account)
		{
			...
			if (round($value,2) != 0)
			{
				...
				$query_result2 = mysqli_query($db,"SELECT * FROM funds WHERE name='$fund'");
